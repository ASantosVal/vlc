...
local function id3_id3_editor.editor.readstring ( str )
	str = str:gsub ( "[%s%z]*$" , "" )
	if #str == 0 then return nil end
	return str:utf8 ( Locale )
end

function id3_id3_editor.editor.find ( fd )
...
function id3_id3_editor.editor.info ( fd , offset )
	--  offset should always be (size of file-128)
	if offset then fd:seek ( "set" , offset ) end
	
	if fd:read ( 3 ) ==  "TAG" then 
		local tags = { }
		tags.title = { id3_id3_editor.editor.readstring ( fd:read( 30 ) ) } 
		tags.artist = { id3_id3_editor.editor.readstring ( fd:read ( 30 ) ) } 
		tags.album = { id3_id3_editor.editor.readstring ( fd:read ( 30 ) ) } 
		tags.date = { id3_id3_editor.editor.readstring ( fd:read ( 4 ) ) }
		...
		if fd:read ( 4 ) == "TAG+" then
			tags.title [ 1 ] = tags.title[1] .. id3_id3_editor.editor.readstring ( fd:read ( 60 ) )
			tags.artist [ 1 ] = tags.artist[1] .. id3_id3_editor.editor.readstring ( fd:read ( 60 ) )
			tags.album [ 1 ] = tags.album[1] .. id3_id3_editor.editor.readstring ( fd:read ( 60 ) )
			tags.speed = { speedindex [ tonumber ( fd:read ( 1 ):byte ( ) ) ] }
			tags.genre [ 2 ] = id3_id3_editor.editor.readstring ( fd:read ( 30 ) )
			do
				local start = id3_id3_editor.editor.readstring ( fd:read ( 30 ) )
				if #start == 6 and start:sub ( 4 , 4 ) == ":" then
					tags["start-time"] = { tostring ( tonumber ( start:sub ( 1 , 3 ) ) * 60 + start:sub ( 5 , 6 ) ) }
				end
				local fin = id3_id3_editor.editor.readstring ( fd:read ( 30 ) )
...
local function id3_id3_editor.editor.twodigityear ( capture )
...
local function id3_id3_editor.editor.guessyear ( datestring )
	local patterns = {
		[".*%f[%d](%d%d%d%d)%f[%D].*"] = { matches = 1 , replace = "%1" }, -- MAGICAL FRONTIER PATTERN (undocumented)
		["^%W*(%d%d)%W*$"] = { matches = 1 , replace = id3_id3_editor.editor.twodigityear }, -- Eg: 70 or '70
		["^%s*%d%d?%s*[/-%s]%s*%d%d?%s*[/-%s]%W*(%d%d)%s*$"] = { matches = 1 , replace = id3_id3_editor.editor.twodigityear }, -- Eg: 20/4/87
		[".*Year%W*(%d%d)%W.-"] = { matches = 1 , replace = id3_id3_editor.editor.twodigityear }, -- Eg: Month: October, Year: 69
		[".*%W(%d%d)%W*$"] = { matches = 1 , replace = id3_id3_editor.editor.twodigityear }, -- Eg: Concert of '97.
	}
...
local function id3_id3_editor.editor.makestring ( str , tolength )
	--str = toid3:iconv ( str )
	str = str:sub ( 1 , tolength )
	return str .. ("\0"):rep ( tolength-#str )
end

function id3_id3_editor.editor.generatetag ( tags )	
	...
	title = id3_id3_editor.editor.makestring ( title [ 1 ] , 30 )
	...
	do -- Artist
		...
		artist = id3_id3_editor.editor.makestring ( artist , 30 )
	end
	...
	album = id3_id3_editor.editor.makestring ( album [ 1 ] , 30 )
	...
	year = id3_id3_editor.editor.makestring ( ( id3_id3_editor.editor.guessyear ( year [ 1 ] ) or "" ) , 4 )
	...
	comment = id3_id3_editor.editor.makestring ( comment , 28 )
	...
	do
		...
		for i , v in ipairs ( genreindex ) do
			if genre:lower ( ):id3_id3_editor.editor.find ( v:lower ( ):gsub ( "%W" , "." ) ) then
				t = i
				break
			end 
		end
...
function id3_editor.edit ( path , tags , inherit )
	local fd , err = ioopen ( path , "rb+" )
	if not fd then return ferror ( err , 3 ) end
	
	if inherit then 
		local currenttags= id3_id3_editor.editor.info ( fd ) -- inherit from current data
	...
	local id3 = id3_id3_editor.editor.generatetag ( tags )
	
	if #id3 ~= 128 then return ferror ( "Unknown error" , 3 ) end
	
	-- Check if file already has an ID3 tag
	local starttag = id3_id3_editor.editor.find ( fd )
