...
FingerprintDialogExt::FingerprintDialogExt(QWidget *parent, intf_thread_t *p_intf,
                                     input_item_t *p_item ) :
    QDialog(parent),
    ui(new Ui::FingerprintDialog), p_r( NULL )
{
...
void FingerprintDialogExt::applyIdentity()
{
    Q_ASSERT( p_r );
    if ( ui->recordsList->currentIndex().isValid() )
        t->apply( p_r, ui->recordsList->currentIndex().row() );
...
void FingerprintDialogExt::handleResults()
{
    p_r = t->fetchResults();
...
FingerprintDialogExt::~FingerprintDialogExt()
{
    if ( t ) delete t;
    if ( p_r ) fingerprint_request_Delete( p_r );
    delete ui;
